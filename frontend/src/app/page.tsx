...
const AGENTS = [{icon:'🎯',name:'Orchestrator',desc:'Decomposes brief and coordinates pipeline'},{icon:'📋',name:'Filing Agent',desc:'Ingests SEC 10-K, 10-Q, 8-K via RAG'},{icon:'🎙️',name:'Earnings Agent',desc:'Scores management tone and Q&A evasion'},{icon:'📊',name:'Comps Agent',desc:'Live peer comparison: P/E, EV/EBITDA, margins'},{icon:'📰',name:'News Agent',desc:'Temporal-decay weighted sentiment scoring'},{icon:'✍️',name:'Synthesis Agent',desc:'Writes bull thesis, bear thesis, valuation'},{icon:'🔍',name:'Critique Agent',desc:'Red-teams every claim, assigns confidence'},{icon:'📎',name:'Citation Agent',desc:'Maps every claim to source and passage'}]
const CHIPS=['AAPL','NVDA','MSFT','GOOGL','TSLA','META','AMZN']
...
export default function Home(){
  ...
  useEffect(()=>{const t=setInterval(()=>setHover(h=>(h+1)%8),2200);return()=>clearInterval(t)},[])
  ...
  return(<div style={S}>
    <style>{`@import url('https://fonts.googleapis.com/css2?family=DM+Sans:wght@300;400;500&family=DM+Mono:wght@500&display=swap');*{box-sizing:border-box;margin:0;padding:0}html{scroll-behavior:smooth}body{-webkit-font-smoothing:antialiased}@keyframes pulse{0%,100%{opacity:1}50%{opacity:.4}}@media(max-width:900px){.hg{grid-template-columns:1fr!important}.mg{grid-template-columns:repeat(2,1fr)!important}.ag{grid-template-columns:repeat(2,1fr)!important}.rg{grid-template-columns:1fr!important}.pg{grid-template-columns:1fr!important}nav a{display:none}}`}</style>
    <nav style={{position:'fixed',top:0,left:0,right:0,zIndex:100,height:64,display:'flex',alignItems:'center',justifyContent:'space-between',padding:'0 48px',background:'rgba(248,247,244,.95)',backdropFilter:'blur(16px)',borderBottom:'1px solid #e8e6de'}}>
      ...
      <div style={{display:'flex',gap:12}}><button style={{fontSize:14,background:'none',border:'1px solid #e8e6de',borderRadius:4,padding:'8px 18px',cursor:'pointer',color:'#3a3a4a'}}>Sign in</button><button style={{fontSize:14,fontWeight:500,background:'#0a0a0f',color:'#fff',border:'none',borderRadius:4,padding:'9px 20px',cursor:'pointer'}}>Get started</button></div>
    </nav>
    <section style={{minHeight:'100vh',display:'flex',alignItems:'center',padding:'120px 48px 80px',position:'relative',overflow:'hidden'}}>
      ...
      <div className="hg" style={{maxWidth:1200,margin:'0 auto',width:'100%',display:'grid',gridTemplateColumns:'1fr 1fr',gap:80,alignItems:'center',position:'relative',zIndex:1}}>
        <div>
          ...
          <h1 style={{fontFamily:'Georgia,serif',fontSize:'clamp(44px,5.5vw,74px)',lineHeight:1.05,letterSpacing:'-.02em',marginBottom:24}}>The research platform<br/><span style={{color:'#2d5a8f',fontStyle:'italic'}}>serious investors</span><br/>use.</h1>
    ...
    <section id="search" style={{padding:'100px 48px',background:'#fff'}}>
      <div style={{maxWidth:760,margin:'0 auto'}}>
        ...
        <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
          {CHIPS.map(t=><button key={t} onClick={()=>setTicker(t)} style={{fontFamily:'monospace',fontSize:12,fontWeight:600,letterSpacing:'.04em',background:ticker===t?'rgba(45,90,143,.06)':'#f8f7f4',border:`1px solid ${ticker===t?'#2d5a8f':'#e8e6de'}`,color:ticker===t?'#2d5a8f':'#3a3a4a',borderRadius:100,padding:'5px 14px',cursor:'pointer'}}>{t}</button>)}
        </div>
      </div>
    </section>
    <section id="agents" style={{padding:'100px 48px',background:'#f8f7f4'}}>
      <div style={{maxWidth:1100,margin:'0 auto'}}>
        <div style={{maxWidth:560,marginBottom:60}}>
          <div style={{fontSize:11,fontWeight:500,letterSpacing:'.1em',textTransform:'uppercase',color:'#c9a84c',marginBottom:12}}>Agent Architecture</div>
          ...
          <p style={{fontSize:15,fontWeight:300,color:'#3a3a4a',lineHeight:1.7}}>Each agent runs in parallel where possible and hands off verified outputs to the next stage.</p>
        </div>
        <div className="ag" style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:14}}>
          {AGENTS.map((a,i)=><div key={a.name} onMouseEnter={()=>setHover(i)} style={{background:'#fff',border:`1px solid ${hover===i?'rgba(201,168,76,.4)':'#e8e6de'}`,borderRadius:10,padding:22,transition:'all .25s',transform:hover===i?'translateY(-3px)':'none',boxShadow:hover===i?'0 12px 32px rgba(10,10,15,.08)':'none',cursor:'default'}}>
            <div style={{width:40,height:40,background:'#f8f7f4',border:'1px solid #e8e6de',borderRadius:8,display:'flex',alignItems:'center',justifyContent:'center',fontSize:18,marginBottom:14}}>{a.icon}</div>
            <div style={{fontSize:14,fontWeight:500,marginBottom:6}}>{a.name}</div>
            <div style={{fontSize:13,fontWeight:300,color:'#3a3a4a',lineHeight:1.6}}>{a.desc}</div>
            <div style={{display:'flex',alignItems:'center',gap:5,marginTop:12}}><span style={{width:5,height:5,borderRadius:'50%',background:'#2a9b6a'}}/><span style={{fontSize:11,fontWeight:500,color:'#2a9b6a'}}>Active</span></div>
          </div>)}
        </div>
      </div>
    </section>
    ...
    <footer style={{padding:48,background:'#0a0a0f',display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:20}}>
      ...
      <div style={{fontSize:12,color:'rgba(255,255,255,.25)'}}>© 2025 VERITY Research Platform</div>
    </footer>
  </div>)
}
